...
#include "nrutil.h"
#include "nr.h"
#define NAME (char *)("Orbit")  //Give your set of ODE's a name
#define NUM 4  //Number of ODE's
#define XINIT {1, 0, 0, 0.1}  //Initial conditions
...
void deriv(double t, double x[], double dxdt[]){
  ...
  dxdt[3] = -2*x[1]/pow(1+2*x[1]*x[1]+2*x[2]*x[2], 1.5);
  dxdt[4] = -2*x[2]/pow(1+2*x[1]*x[1]+2*x[2]*x[2], 1.5);
